...
export default function Toast02({
  ...
  const typeIcon = (type: string): ReactElement<any> => {
    switch (type) {
      ...
      default:
        return (
          <svg className="shrink-0 fill-current text-primary-500 mt-[3px] mr-3" width="16" height="16" viewBox="0 0 16 16">
  ...
  const typeColor = (type: string): string => {
    switch (type) {
      ...
      default:
        return 'bg-primary-100 text-gray-700'
    }
  }
    ...
    <>
      {open &&
        <div className={className} role="alert">
          <div className={`inline-flex min-w-[20rem] px-4 py-2 rounded-lg text-sm ${typeColor(type)}`}>
